...
class track : public TObject{
 public:
  ...
  inline void getIsTrackerForTrack(Bool_t isTrackerForTrack[]) const { for(Int_t i=0; i<kNTrackers; i++) isTrackerForTrack[i] = fIsTrackerForTrack[i]; } 
  ...
  inline void  getTrackersPathMomentum(Double_t trackersPathMomentum[]) {for(Int_t i=0; i<kNTrackers; i++) trackersPathMomentum[i]=fTrackersPathMomentum[i];} 
 ...
 private:
  ...
  Bool_t fIsTrackerForTrack[kNTrackers];
