...
const Double_t kChi2PerMdimCut=1e10; // For each measurement in the filter,  DeltaChi2 limit = kChi2PerMdimCut * kMdim
...
const Int_t kPlaneId[kNTotPlanes] = {0, 1, 2, 3 ,4};
...
const Double_t kPlanePosition[kNTotPlanes]={0, 62., 62.*2, 62*3, 62.*4}; // mm; order corresponds planeId
const Double_t kFrontPlaneShift[kNTotPlanes]={0, 4.576, 4.576, 4.576, 4.576}; // mm; the first element = 0 for the initial plane
const Double_t kBackPlaneShift[kNTotPlanes]={0, 10.811, 10.811, 10.811, 10.811}; // mm; the first element = 0 for the initial plane

const Int_t kNMatType = 5;  //Total number of material types
enum matType{kAirMat = 0, kTracker1Mat, kTracker2Mat, kTracker3Mat, kTracker4Mat}; // 0 is for air, initial site is supposed to be surrounded by air; 1, 2, 3 ... must be for trackers; then, following numbers are for detecotrs which are not trackers, such as ToFs
...
const Double_t kInitMomentum=115; //MeV/c
...
const Double_t kDafWeightThreshold = 1.e-5;

// Parameters for propagation
const Bool_t kIsMSOn = kTRUE; // if musti-scattering turns on
const Bool_t kIsDEDXOn = kTRUE; // if energy loss turns on
