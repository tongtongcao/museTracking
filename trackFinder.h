...
class trackFinder: public particle{
 public:
  ...
  Bool_t evaluateTracking();
  Bool_t processTrackingAfterRomoveOneTracker();
  Bool_t processTrackingAfterRomoveTwoTrackers();
  ...
  struct dafInfo{
    ...
    Bool_t isTrackerForSeedArray[kNTrackers];

    dafInfo() {}
    dafInfo(vector<planeHit> hitVectArray[kNTrackers], vector<Double_t> weightVectArray[kNTrackers], Bool_t isForSeedArray[kNTrackers]) {
      for(Int_t i=0; i<kNTrackers; i++){
	trackerHitVectArray[i] = hitVectArray[i];
	dafWeightVectArray[i] = weightVectArray[i];
	isTrackerForSeedArray[i] = isForSeedArray[i];
      }
    }
  ...
  Bool_t calcDafEffectiveMeas(planeHit& effectiveHit, vector<planeHit> hitVect, vector<Double_t> weightVect);
